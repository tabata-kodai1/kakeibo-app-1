...
| 「検索」ボタン | ボタン | 入力条件で `GET /api/entries` を再取得 | F-03 |
...
| 「保存」ボタン | ボタン | 追加は `POST /api/entries`、編集は `PUT /api/entries/{id}`。成功でモーダルを閉じ一覧を更新 | F-04, F-05 |
...
| 「削除する」ボタン | ボタン | `DELETE /api/entries/{id}` を実行し、閉じて一覧を更新 | F-07 |
