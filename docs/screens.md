...
| 期間注記 | 表示 | 明細一覧が対象月の外を含む期間検索中のみ「表示中の期間: 2026年9月」を出し、明細と期間が異なることを示す | F-08 |
...
| 要素 | 種別 | 挙動 | 対応機能 |
| --- | --- | --- | --- |
| 件数表示 | 表示 | 「明細 12件」。絞り込み中は「12件中 3件」。対象月の外を含む期間検索中は母数を出さず「該当 3件（2026/01/01〜2026/09/30）」。**母数は `GET /api/summary` の `entry_count`**、該当件数は明細一覧の配列長から取る | F-04, F-08 |
